...
RegisterID* ApplyFunctionCallDotNode::emitBytecode(BytecodeGenerator& generator, RegisterID* dst)
{
    ...
    generator.emitJumpIfNotFunctionApply(function.get(), realCall.get());
    {
        if (mayBeCall) {
            ...
            // for apply, the arguments are loaded and arranged before the call is actually made
            // we allocate here the temporary sampler register and pass it into the callVarArgs
            // to be used in op_sampler_will_call and op_sampler_did_call
            RefPtr<RegisterID> samplerNodeRegister;
            if (generator.shouldEmitSamplerHooks()) {
                samplerNodeRegister = generator.newTemporary();
            }
#endif
...
#if ENABLE(APOLLO_PROFILER)
            generator.emitCallVarargs(finalDestination.get(), realFunction.get(), thisRegister.get(), argsCountRegister.get(), 
                divot(), startOffset(), endOffset(), samplerNodeRegister.get());
#else
            generator.emitCallVarargs(finalDestination.get(), realFunction.get(), thisRegister.get(), argsCountRegister.get(), 
...
#if PLATFORM(APOLLO)
    generator.emitDebugHook(WillLeaveCallFrameFromReturnCommand, firstLine(), lastLine());
#else
...
#if PLATFORM(APOLLO)
    generator.emitDebugHook(WillLeaveCallFrameFromFunctionEnd, firstLine(), lastLine());
#else
    generator.emitDebugHook(WillLeaveCallFrame, firstLine(), lastLine());
#endif
