...
namespace JSC {
    ...
    PassRefPtr<ParsedNode> Parser::parse(JSGlobalData* globalData, Debugger* debugger, ExecState* debuggerExecState, const SourceCode& source, int* errLine, UString* errMsg)
    {
		...
		// check if needed
		// APOLLO {
        // Moved this block before the call to ParsedNode::create()
        // because "create" clears the m_funcDeclarations vector that
        // is used by the debugger in sourceParsedFunctions.
#if PLATFORM(APOLLO)
        if (debugger && !ParsedNode::scopeIsFunction) {
            debugger->sourceParsed(debuggerExecState, source, *errLine, *errMsg);
            if (m_funcDeclarations) {
                debugger->sourceParsedFunctions(debuggerExecState, source.provider()->asID(), m_funcDeclarations);
            }
        }
#endif
        // } APOLLO	
