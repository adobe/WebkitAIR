...
namespace JSC {
...
#if PLATFORM(APOLLO)
        ASSERT(assign);
        if(LIKELY(!assign->isFuncExprNode()))
			return;
		
		updateFuncExprNodeProfilerIdent(assign, 0, name);
#endif
    }
...
#if PLATFORM(APOLLO)
        ASSERT(right);
        if(LIKELY(!right->isFuncExprNode()))
			return;

		updateFuncExprNodeProfilerIdent(right, 0, ident);
#endif
    }
        ...
        , m_rightHasAssignments(rightHasAssignments)
    {
        ...
        updateFuncExprNodeProfilerIdent(right, base, ident);
#endif
    }
