...
namespace JSC {
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { 
            return level == EvalAny; 
        }
#endif
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalLiteral; }
#endif
    ...
    public:
        ...
        ThrowableExpressionData(unsigned divot, unsigned startOffset, unsigned endOffset)
            : m_divot(divot)
            , m_startOffset(static_cast<uint16_t>(startOffset))
            , m_endOffset(static_cast<uint16_t>(endOffset))
        {
        }
    ...
    public:
        ...
        void setSubexpressionInfo(uint32_t subexpressionDivot, uint16_t subexpressionOffset)
        {
            ...
            if ((divot() - subexpressionDivot) & ~0xFFFF) // Overflow means we can't do this safely, so just point at the primary divot
                return;
            m_subexpressionDivotOffset = static_cast<uint16_t>(divot() - subexpressionDivot);
    ...
    public:
        ...
        void setSubexpressionInfo(uint32_t subexpressionDivot, uint16_t subexpressionOffset)
        {
            ...
            if ((subexpressionDivot - divot()) & ~0xFFFF) // Overflow means we can't do this safely, so just point at the primary divot
                return;
            m_subexpressionDivotOffset = static_cast<uint16_t>(subexpressionDivot - divot());
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalReadOnly; }
#endif 		
...
#if PLATFORM(APOLLO)
		/* Adding a virtual destructor to get it to compile with the isSafeForEval virtual */
		virtual ~ElementNode(){ }
#endif
    };
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalLiteral && (!m_element || m_element->isSafeForEval(level)); }
#endif
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalLiteral && (!m_list || m_list->isSafeForEval(level)); }
#endif
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalReadOnly && m_base->isSafeForEval(level) && m_subscript->isSafeForEval(level); }
#endif		
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalReadOnly && m_base->isSafeForEval(level); }
#endif		
...
#if (__clang_major__ > 4) || ((__clang_major__ == 4) && (__clang_minor__ >= 2))
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalLiteral && m_expr->isSafeForEval(level); }
#endif		
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalConstantExpr && m_expr->isSafeForEval(level); }
#endif
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalConstantExpr && expr()->isSafeForEval(level); }
#endif	
    private:
...
#if PLATFORM(APOLLO)
        // allow for literals since lexer doesn't deal with negatives
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalLiteral && expr()->isSafeForEval(level); }
#endif
    };
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalConstantExpr && m_expr1->isSafeForEval(level) && m_expr2->isSafeForEval(level); }
#endif		
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalConstantExpr && m_logical->isSafeForEval(level) &&
		m_expr1->isSafeForEval(level) && m_expr2->isSafeForEval(level); }
#endif
    private:
        virtual RegisterID* emitBytecode(BytecodeGenerator&, RegisterID* = 0);
...
#pragma clang diagnostic pop
    ...
    public:
        ...
        bool hasInitializer() const { return m_init != 0; }
...
#if PLATFORM(APOLLO)
        // TODO: apollo integrate -> can we expose it differently?
        Vector<StatementNode*> statements() const { return m_statements; }
#endif
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel /*level*/) { return true; }
#endif		
    private:
        virtual RegisterID* emitBytecode(BytecodeGenerator&, RegisterID* = 0);
...
#pragma clang diagnostic pop
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalReadOnly && m_expr->isSafeForEval(level) && m_statement->isSafeForEval(level); }
#endif
    private:
        virtual RegisterID* emitBytecode(BytecodeGenerator&, RegisterID* = 0);
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalReadOnly && m_statement->isSafeForEval(level); }
#endif
    private:
        virtual RegisterID* emitBytecode(BytecodeGenerator&, RegisterID* = 0);
    ...
    public:
        ...
        bool usesArguments() const { return (m_features & ArgumentsFeature) != 0; }
        void setUsesArguments() { m_features |= ArgumentsFeature; }
        bool usesThis() const { return (m_features & ThisFeature) != 0; }
        bool needsActivation() const { return (m_features & (EvalFeature | ClosureFeature | WithFeature | CatchFeature)) != 0; }
...
#if PLATFORM(APOLLO)
        // profiler functions only
        void setContextualName(const UString& name) { m_profilerName = name; }
        UString contextualName() const { return m_profilerName; }
#endif
    private:
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalReadOnlyAndFunctions; }
#endif
    private:
        virtual RegisterID* emitBytecode(BytecodeGenerator&, RegisterID* = 0);
...
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalAny; } // because it defines a binding
#endif
    private:
        virtual RegisterID* emitBytecode(BytecodeGenerator&, RegisterID* = 0);
    ...
    private:
        virtual RegisterID* emitBytecode(BytecodeGenerator&, RegisterID* = 0);
#if PLATFORM(APOLLO)
        virtual bool isSafeForEval(EvalLevel level) { return level >= EvalReadOnly && m_expr->isSafeForEval(level) && m_block->isSafeForEval(level); }
#endif
        ExpressionNode* m_expr;
