...
namespace JSC {
...
#if PLATFORM(APOLLO) && COMPILER(MSVC9_OR_LOWER)
            m_other.structApollo.m_finalizerCallback = finalizer;
            m_other.structApollo.m_finalizerContext = context;
#else
...
#if PLATFORM(APOLLO) && COMPILER(MSVC9_OR_LOWER)
            if (!m_fiberCount && m_other.structApollo.m_finalizerCallback)
                m_other.structApollo.m_finalizerCallback(this, m_other.structApollo.m_finalizerContext);
#else
    ...
    private:
        ...
        } JSStringFinalizerStructApollo;
        struct JSStringFinalizerStruct {
            JSStringFinalizerStruct() { structApollo.m_finalizerCallback = 0; }
            union {
                mutable RopeImpl::Fiber m_fibers[s_maxInternalRopeLength];
                JSStringFinalizerStructApollo structApollo;
            };
        } m_other;
    ...
    // The below function must be called by any inline function that invokes a JSString constructor.
        ...
        inline JSString* fixupVPtr(JSGlobalData* /*globalData*/, JSString* string) { 
            string->setVPtr(JSGlobalData::jsStringVPtr); 
            return string; 
        }
    #else
    ...
    #if COMPILER(MSVC) && !defined(BUILDING_JavaScriptCore)
        inline JSString* fixupVPtr(JSGlobalData* globalData, JSString* string) { 
            string->setVPtr(globalData->jsStringVPtr); 
            return string; 
        }
    #else
        inline JSString* fixupVPtr(JSGlobalData*, JSString* string) { return string; }
    ...
    {
        if (c <= 0xFF)
            return globalData->smallStrings.singleCharacterString(globalData, c & 0xFF);
    ...
    inline JSString* jsString(JSGlobalData* globalData, const UString& s)
    {
        ...
        if (size == 1) {
            UChar c = s.data()[0];
            if (c <= 0xFF)
                return globalData->smallStrings.singleCharacterString(globalData, static_cast<unsigned char>(c));
        }
        return fixupVPtr(globalData, new (globalData) JSString(globalData, s));
    }
...
#if PLATFORM(APOLLO)
    inline bool isJSString(JSGlobalData* /*globalData*/, JSValue v) { 
        return v.isCell() && v.asCell()->vptr() == JSGlobalData::jsStringVPtr; 
    }
#else
    inline bool isJSString(JSGlobalData* globalData, JSValue v) { 
        return v.isCell() && v.asCell()->vptr() == globalData->jsStringVPtr; 
    }
#endif
