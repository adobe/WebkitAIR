...
#if PLATFORM(APOLLO)
    // mihnea: specifying VM_TAG_FOR_COLLECTOR_MEMORY fails the allocation, please investigate
...
#if ENABLE(APOLLO_PROFILER)
    //duplicates the original iteration for maximum speed
    //unlikely instructs the linux? compiler that we expect to have this SamplerApollo::enabled returning 0 most of the times
    if (UNLIKELY(SamplerApollo::enabled()))
    {
        ...
        for ( ; it != end; ++it)
        {
            if (!(*it)->isNumber())
            {  
                //this also happens for dummy cells - having no real corespondent
                //we should be fine since such cells should not be found in sampler's livenodes list

                sampler->unregisterNodeHelper((void*)*it);
            }
            (*it)->~JSCell();
        }
    }
    ...
    if (UNLIKELY(SamplerApollo::enabled()))
    {
        ...
        for (ProtectCountSet::iterator it = protectedValuesCopy.begin(); it != protectedValuesEnd; ++it)
        {
            ...
            {  
                ...
                sampler->unregisterNodeHelper((void*)it->first);
            }
            it->first->~JSCell();
        }
    }
...
#if ENABLE(APOLLO_PROFILER)
void Heap::samplerRegisterJSCell(void* cell, size_t s)
{
    if (UNLIKELY( SamplerApollo::enabled() ))
	{
        JSGlobalObject* globalObject = m_globalData->dynamicGlobalObject;

        if (UNLIKELY(globalObject == 0))
        {
            if (JSGlobalObject* head = m_globalData->head) {
                if (head->next() == head) {
                    // this is the first global object
                    globalObject = head;
                } else {
                    // the last global object is always the object next to head
                    globalObject = head->next();
                }
            } else {
                // The object being created right now is going to be the GlobalObject
                // the logic in SamplerApollo will fix that, because now it's too early
                // to get into the new/uninitialized JSGlobalObject
            }
        }
		
        (*SamplerApollo::enabledSamplerReference())->registerJSCell(globalObject, 
            reinterpret_cast<JSCell*>(cell), s);
	}
}

void Heap::samplerUnregisterAllJSCell()
{
    if (UNLIKELY( SamplerApollo::enabled()))
    {
        SamplerApollo* sampler = *SamplerApollo::enabledSamplerReference();

        // iterate all over the jscells in the heap and unregister only those that are not numbers
        DeadObjectIterator it(m_heap, m_heap.nextBlock, m_heap.nextCell);
        DeadObjectIterator end(m_heap, m_heap.usedBlocks);
        for ( ; it != end; ++it) {
            JSCell* cell = *it;
            ...
            if (cell->isNumber())
                continue;
#endif
            sampler->unregisterNodeHelper((void*)cell);
        }
    }
}
...
#if ENABLE(APOLLO_PROFILER)
void* Heap::allocate(size_t s, bool isNumber)
#else
void* Heap::allocate(size_t s, bool)
#endif
{
            ...
            if (!block->marked.get(m_heap.nextCell)) { // Always false for the last cell in the block
                ...
                if (UNLIKELY(SamplerApollo::enabled()))
                    ...
                    {  
                        ...
                        (*SamplerApollo::enabledSamplerReference())->unregisterNodeHelper((void*)cell);
                    }
#endif
                imp->~JSCell();
...
#if ENABLE(APOLLO_PROFILER)
    const bool apolloProfilerEnabled = SamplerApollo::enabled();
#endif
...
#if ENABLE(APOLLO_PROFILER)
        if (UNLIKELY(apolloProfilerEnabled))
        {
            if (!cell->isNumber())
            {  
                //this also happens for dummy cells - having no real corespondent
