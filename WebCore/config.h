...
#if !PLATFORM(CHROMIUM) && OS(WINDOWS) && !defined(BUILDING_WX__) && !COMPILER(GCC) && !PLATFORM(APOLLO)
...
#if PLATFORM(MAC) || PLATFORM(APOLLO)
    #define WEBCORE_NAVIGATOR_VENDOR "Adobe Systems Incorporated"
...
/*PLATFORM(APOLLO) definitions in one place*/
#if PLATFORM(APOLLO)
    ...
    #undef JS_EXPORTCLASS
    #define JS_EXPORTCLASS

    #undef WEBKIT_EXPORTDATA
    #define WEBKIT_EXPORTDATA
