...
CachedResource::~CachedResource()
{
	...
	// remove all the weak references to this object as we go dead
	HashSet<CachedResourceHandleBase*>::iterator end = m_weakHandlesToInvalidate.end();
    for (HashSet<CachedResourceHandleBase*>::iterator it = m_weakHandlesToInvalidate.begin(); it != end; ++it) {
...
#if ENABLE(APOLLO_CLEAR_CACHE)
		// decrease the handle count only for the weak references 
...
{
	// the only differecen between this method and the WebKit registerHandle implementation
	// is the addition of the weakReference boolean needed for DocLoaders that should
	// not keep a reference to this object
	const bool weakReference = h->isWeakReference();
	
	if (!weakReference) 
		++m_handleCount;
	else
		m_weakHandlesToInvalidate.add(h);
...
void CachedResource::unregisterHandle(CachedResourceHandleBase* h)
{
	...
	if (!weakReference) { 
		...
		m_weakHandlesToInvalidate.remove(h);
	}
	
	if (m_resourceToRevalidate) 
		m_handlesToRevalidate.remove(h); 
	
    if (!m_handleCount && !weakReference) 
        deleteIfPossible(); 
}
#endif
