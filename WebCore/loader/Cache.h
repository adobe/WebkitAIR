...
public:
    ...
    void clear()
    {
        // We use this method instead of setDisable(true/false) because
        // we don't want to remove resources that are still in use and the 
        // simple call to prune will not suffice because it will have an effect only
        // if the m_maxDeadCapacity is reached.
        
        // This method is used from WebViewImpl::~WebViewImpl
        pruneDeadResources(); // Prune dead first, in case it was "borrowing" capacity from live.
        pruneLiveResources();
    }
#endif
