...
public:
...
#if PLATFORM(APOLLO)
    CachedScript* requestScript(const String& url, const String& charset, const String& scriptType);
#else
	...
	// keep only weak references to Cached Resource from DocLoader
	// see bug: #2647006 for details
    typedef HashMap<String, CachedResourceHandle<CachedResource, true> > DocumentResourceMap;
#else
