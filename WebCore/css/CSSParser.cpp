...
extern const struct props *findProp (register const char *str, register unsigned int len);
#include "CSSPropertyNames.cpp"
extern const struct css_value *findValue (register const char *str, register unsigned int len);
...
#if PLATFORM(APOLLO)
        // for -webkit-background-clip we have to allow border|content|padding and disallow text
        bool isWebKitBackgroundClipText = false;
        // for -webkit-mask-clip we have to allow all values except text
        bool isWebKitMaskClipText = false;
#endif
                    ...
                    if (val->id == CSSValueBorder || val->id == CSSValuePadding || val->id == CSSValueContent ||
                        ...
                        ((propId == CSSPropertyWebkitBackgroundClip || propId == CSSPropertyWebkitMaskClip) &&
                         (val->id == CSSValueText || val->id == CSSValueWebkitText))) {
...
#if PLATFORM(APOLLO)
                            if ((CSSPropertyWebkitBackgroundClip == propId) && (CSSValueText == val->id)) {
                                isWebKitBackgroundClipText = true;
                                // this is set in order to move to the next value in an enum and skip this one
                                allowComma = true;
                            ...
                            if ((CSSPropertyWebkitMaskClip == propId) && (CSSValueText == val->id)) {
                                isWebKitMaskClipText = true;
                                // this is set in order to move to the next value in an enum and skip this one
                                allowComma = true;
                                // move to the next element of the list
                                m_valueList->next();
...
#if PLATFORM(APOLLO)
            // skip this item and read on
            if (isWebKitBackgroundClipText || isWebKitMaskClipText)
                continue;
#endif
