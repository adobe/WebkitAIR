...
#if PLATFORM(APOLLO)
static CSSRuleSet* defaultApolloCSSStyle;
#endif
...
#if PLATFORM(APOLLO)
        defaultApolloCSSStyle = new CSSRuleSet;
        defaultApolloCSSStyle->addRulesFromSheet(parseUASheet(apolloUserAgentStyleSheet, sizeof(apolloUserAgentStyleSheet)), screenEval());
#endif
    }
...
void CSSStyleSelector::matchRulesForList(CSSRuleDataList* rules, int& firstRuleIndex, int& lastRuleIndex)
{
    ...
    for (CSSRuleData* d = rules->first(); d; d = d->next()) {
        ...
        if (m_checker.m_sameOriginOnly && !m_checker.m_document->securityOrigin()->canRequestExt(rule->baseURL(), m_checker.m_document))
            continue; 
...
#if PLATFORM(APOLLO)
	Frame* frame = m_checker.m_document->frame();

	// investigate if this assert fires for versions greater than WEBKIT_APOLLO_AIR_1_5
	ASSERT(frame);

	if (frame)
	{
		WebHost* webHost = FrameLoaderClientApollo::clientApollo(frame)->webHost();
		if(webHost->m_pVTable->isPlatformAPIVersion(webHost,WEBKIT_APOLLO_AIR_1_5))
			matchRules(defaultApolloCSSStyle, firstUARule, lastUARule);
	}
#endif
}
