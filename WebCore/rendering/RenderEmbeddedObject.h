...
// apollo - compile - integrate - Safari 5.0.3
// olaru - adding a forward declaration for Path for a fast fix of compile time errors
// in implementation files. I have not yet tracked down the difference between the WebKit trunk
// and why the build succeeds there (where exactly is Path.h included from in their cpp files).
// My suspicion is that it is from headers between feature guards that are not enabled in
// the Apollo build - most likely SVG.
// Remove the Path forward declaration when the inclusion path is tracked down and can be fixed.
class Path; // APOLLO
class MouseEvent;
