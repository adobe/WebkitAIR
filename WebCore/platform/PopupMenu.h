...
#if PLATFORM(APOLLO)
    void updatePosition();
	void populate( WebPopupWindow* webPopupMenu );
	void getPopupPositionInClient( int* viewportX, int* viewportY );
	WebHost* getWebHost( FrameView* frameView );
...
#elif PLATFORM(APOLLO)
	WebHost* m_webHost;
	WebPopupWindow* m_webPopupWindow;
#elif PLATFORM(WX)
    wxMenu* m_menu;
