...
#include <wtf/RetainPtr.h>
    ...
    || (OS(WINDOWS) && PLATFORM(WX)) || (PLATFORM(APOLLO) && OS(WINDOWS))
#include <usp10.h>
#endif
...
#if PLATFORM(MAC) || (PLATFORM(CHROMIUM) && OS(DARWIN)) || (OS(DARWIN) && PLATFORM(APOLLO))
...
#if PLATFORM(WIN) || (OS(WINDOWS) && PLATFORM(WX)) || (PLATFORM(APOLLO) && OS(WINDOWS))
...
#if PLATFORM(MAC) || (OS(DARWIN) && PLATFORM(APOLLO))
#if defined(BUILDING_ON_TIGER) || (OS(DARWIN) && PLATFORM(APOLLO))
public:
    void* m_styleGroup;
private:
#endif
