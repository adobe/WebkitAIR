...
#if PLATFORM(APOLLO)
#include <apollo/RenderCheckbox.h>
#include <apollo/RenderRadio.h>

// This is needed because Adobe AIR does not fully implement RenderSlider.
// When you remove this, also change WebCore/css/html.css.
#define ENABLE_APOLLO_RANGE_AS_TEXT_FIELD 1
#endif
...
bool HTMLInputElement::valueMissing() const
{
    ...
    case NUMBER:
    case PASSWORD:
#if ENABLE(APOLLO_RANGE_AS_TEXT_FIELD)
    case RANGE:
#endif
    case SEARCH:
    case TELEPHONE:
...
#endif
{
    switch (inputType()) {
        ...
        case RADIO:
            return new (arena) RenderRadio(this);
#else
        case CHECKBOX:
        case RADIO:
...
bool HTMLInputElement::appendFormData(FormDataList& encoding, bool multipart)
{
    ...
    case WEEK:
        ...
        if (!multipart) {
            ...
            if (!numFiles) {
#if PLATFORM(APOLLO)
                encoding.appendBlob(name(), File::create("", document()));
#else
...
#if PLATFORM(APOLLO)
        m_fileList->append(File::create(paths[i], document()));
#else
