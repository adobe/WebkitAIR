...
{
#if PLATFORM(APOLLO)
    m_onChangeWillFire = fireOnChangeNow;
#endif
...
#if PLATFORM(APOLLO)
    int oldSize = m_data.size();
    if ((oldSize == 0) && (size != oldSize) && m_onChangeWillFire)
        // the control will turn from a drop down to a list box and this will cause a crash
        return;
#endif
