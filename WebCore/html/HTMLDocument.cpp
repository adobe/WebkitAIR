...
extern const struct PubIDInfo *findDoctypeEntry (register const char *str, register unsigned int len);
...
Tokenizer *HTMLDocument::createTokenizer()
{
    ...
    if (frame()) {
		ASSERT(frame()->loader());
		if (FrameLoaderClient *loaderClient = frame()->loader()->client())
			reportErrors = loaderClient->isDebuggingEnabled();
    }
#endif
#if ENABLE(INSPECTOR)
