...
void HTMLAnchorElement::defaultEventHandler(Event* evt)
{
    ...
    if (isLink() && (evt->type() == eventNames().clickEvent || (evt->type() == eventNames().keydownEvent && focused()))) {
        ...
        if (!evt->defaultPrevented() && document()->frame()) {
            // Security: this should be a real event and not a replayed or synthesized event.
            Page *page = document()->frame()->page();
            if (page && page->chrome()->processingUserGesture())
                document()->frame()->loader()->urlSelected(document()->completeURL(url), getAttribute(targetAttr), evt, 
                    false, false, true, hasRel(RelationNoReferrer) ? NoReferrer : SendReferrer);
        }
#else
        if (!evt->defaultPrevented() && document()->frame())
            document()->frame()->loader()->urlSelected(document()->completeURL(url), getAttribute(targetAttr), evt, 
