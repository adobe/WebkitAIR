...
extern const struct Entity* findEntity(register const char* str, register unsigned int len);
    ...
    if (!inViewSourceMode()) {
        if (!m_scriptTagSrcAttrValue.isEmpty() && m_doc->frame()) {
            ...
            if (!m_parser->skipMode() && !followingFrameset) {
				...
				// apollo integrate -> check here new condition
#if PLATFORM(APOLLO)
                if (!m_parserStopped && (cs = m_doc->docLoader()->requestScript(m_scriptTagSrcAttrValue, m_scriptTagCharsetAttrValue, m_scriptTagTypeAttrValue)))
#else
...
#if PLATFORM(APOLLO)
    JSC::UString scriptURL = sourceCode.jsSourceCode().provider()->url();
    KURL scriptKURL(ParsedURLString, ustringToString(scriptURL));
    if (loadScriptRestricted(m_doc->frame(), scriptKURL))
        return state;
#endif
...
#if PLATFORM(APOLLO)
            // In case of self closing script tag, WebKit will automatically add a closing tag 
            // (later in scriptHandler method), but it will never add the begin tag to the stack,
            // so the closing tag will not find it's begining tag and throw a false HTML error.
            // I've set the flat to false, so that it will be included in the stack 
            // (see HTMLParser::insertNode).
            if (isSelfClosingScript)
                m_currentToken.selfClosingTag = false;
#endif
...
#if PLATFORM(APOLLO)
                        m_scriptTagTypeAttrValue = static_cast<HTMLScriptElement*>(n.get())->type();
#endif
                    }
                    if (beginTag) {
