...
sub GenerateHeader
{
    ...
    if ($numCustomFunctions > 0) {
        push(@headerContent, "\n    // Custom functions\n");
        push(@headerContent, "#ifdef __clang__\n");
        push(@headerContent, "#pragma clang diagnostic push\n");
        push(@headerContent, "#pragma clang diagnostic ignored \"-Woverloaded-virtual\"\n");
        ...
        push(@headerContent, "#ifdef __clang__\n");
        push(@headerContent, "#pragma clang diagnostic pop\n");
        push(@headerContent, "#endif\n");
    }

#BEGIN PLATFORM(APOLLO)
	if ($interfaceName eq "Node") {
        push(@headerContent, "#if ENABLE(APOLLO_PROFILER)\n");
		push(@headerContent, "    virtual void getMembersForProfiler(JSC::ProfilerMembersList*);\n");
        push(@headerContent, "#endif\n");
	}
#END PLATFORM(APOLLO)
...
#BEGIN PLATFORM(APOLLO)
	if ($interfaceName eq "Node") {
	    $implIncludes{"sampler/ProfilerMembers.h"} = 1;
        push(@implContent, "#if ENABLE(APOLLO_PROFILER)\n");
		push(@implContent, "void ${className}::getMembersForProfiler(JSC::ProfilerMembersList* membersList)\n{\n");
	    push(@implContent, "    membersList->append(\"[Wrapped DOM Object]\", impl());\n\n");
		push(@implContent, "    Base::getMembersForProfiler(membersList);\n");
		push(@implContent, "}\n\n");
        push(@implContent, "#endif\n\n");
	}
#END PLATFORM(APOLLO)
