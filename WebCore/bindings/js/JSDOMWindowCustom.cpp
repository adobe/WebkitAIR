...
#if PLATFORM(APOLLO)
    if (!shouldAllowNavigation(exec, impl()->frame(), FrameLoader::kDoNotCheckUserGestureIfTargetIsNull))
#else
...
#if PLATFORM(APOLLO)
    createWindowSetOpener(openerFrame, newFrame);
#else
...
#if PLATFORM(APOLLO)
    if (isLocationChangeSafe(exec, url, newWindow)) {
#else
...
#if PLATFORM(APOLLO)
        if (!shouldAllowNavigation(exec, frame, FrameLoader::kDoNotCheckUserGestureIfTargetIsNull))
#else
        if (!shouldAllowNavigation(exec, frame))
#endif
            return jsUndefined();
...
#if PLATFORM(APOLLO)
    if (settingStringTimerRestricted(args.at(0), impl()->frame()))
        return throwError(exec, GeneralError, "Adobe AIR runtime security violation for JavaScript code in application security sandbox (window.setTimeout)");
#endif
...
#if PLATFORM(APOLLO)
    if (settingStringTimerRestricted(args.at(0), impl()->frame()))
        return throwError(exec, GeneralError, "Adobe AIR runtime security violation for JavaScript code in application security sandbox (window.setInterval)");
#endif
