...
#if PLATFORM(APOLLO)
     // we validated that it was OK to parse this when the JSLazyEventListener was created,
     // so go ahead and allow parsing
        
     JSDOMWindow* window = static_cast<JSDOMWindow*>(globalObject);
     bool saveDefiningEventHandler = window->definingEventHandler();
     window->setDefiningEventHandler(true);
#endif
...
// apollo integrate
// check here where to call the below function
// end apollo integrate
#if PLATFORM(APOLLO)
        window->setDefiningEventHandler(saveDefiningEventHandler);
#endif
