...
#else
        reportApolloException(exec, comp.value(), m_frame);
#endif
...
#if PLATFORM(APOLLO)
    m_frame->loader()->dispatchWindowObjectDead();
#endif
...
bool ScriptController::processingUserGesture(DOMWrapperWorld* world) const
{
	...
	// Safari 5 integration -> does this belong here, should be moved in UserGestureIndicator?
#if PLATFORM(APOLLO)
    // It's up to the chrome to decide whether we are currently processing a user gesture.
    // In fact, it's really up to the chrome to decide what a gesture is anyway.
    // Moreover, the event inspection code below is vulnerable to synthesized and replayed events.
