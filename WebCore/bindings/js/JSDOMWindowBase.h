...
#if PLATFORM(APOLLO)
        // Separate callouts for eval and new Function, because the policies may differ
        virtual JSC::EvalLevel currentEvalLevel() const;
        virtual bool canConstructFunctions() const;
        void setDefiningEventHandler(bool definingEventHandler);
        bool definingEventHandler() const;
        virtual void *platformInterpreterContext();
        virtual JSC::UString typeName() const;
#endif
