...
Node::~Node()
{
#if PLATFORM(APOLLO) && ENABLE(DOM_SAMPLING)
    // if this fails, it means there is another call to delete instead of calling destroy
    ASSERT(m_deletedFromDestroy);
#endif
#ifndef NDEBUG
...
bool Node::dispatchEvent(PassRefPtr<Event> prpEvent)
{
	...
	// apollo integrate -> do we need this?
    // check if we have a document before going further with dispatch
...
// olaru:
// compile error - function is defined but not used - due to being used only inside
// some inspector code below; it is not declared in Node.h
// putting the function inside #if ENABLE(INSPECTOR)
#if ENABLE(INSPECTOR)
...
void Node::defaultEventHandler(Event* event)
{
    ...
    if (eventType == eventNames().keydownEvent || eventType == eventNames().keypressEvent) {
        ...
        // code from EventHandler::handleMousePressEvent moved here to prevent loosing mouseDown event
        // for middle mouse click (see WebKit Bug 32303/ Watson 2505430)
