...
Document::Document(Frame* frame, bool isXHTML, bool isHTML)
    ...
    , m_destroyWidgetsSoonTimer(this, &Document::destroyWidgetsSoonTimerFired)
#endif
    , m_domtree_version(0)
...
#if PLATFORM(APOLLO)
    m_processingDOMContentLoadedEvent = false;
#endif
...
#if PLATFORM(APOLLO) && ENABLE(NETSCAPE_PLUGIN_API)
    // clear the plugins before their parents
    m_widgetsRegisteredForDestroy.clear();
#endif
...
void Document::recalcStyle(StyleChange change)
{
    ...
    SAMPLER_CONTEXT(&JSC::SamplerContextPool::builtins.cssStyle);
#endif
...
#if ENABLE(APOLLO_PROFILER)
    SAMPLER_CONTEXT( m_tokenizer->isHTMLTokenizer() ? &JSC::SamplerContextPool::builtins.htmlParse : &JSC::SamplerContextPool::builtins.xmlParse );
#endif    
...
void Document::finishedParsing()
{
    setParsing(false);
#if PLATFORM(APOLLO)
    m_processingDOMContentLoadedEvent = true;
#endif
