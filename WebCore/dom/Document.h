...
#if PLATFORM(APOLLO)
    DEFINE_ATTRIBUTE_EVENT_LISTENER(dominitialize);
#endif
...
#if PLATFORM(APOLLO) && ENABLE(NETSCAPE_PLUGIN_API)
    void destroyWidgetSoon(Widget* widget);
#endif
...
#if PLATFORM(APOLLO)
    bool processingDOMContentLoadedEvent() const { return m_processingDOMContentLoadedEvent; }
#endif
...
private:
    ...
    void updateBaseURL();
#if PLATFORM(APOLLO) && ENABLE(NETSCAPE_PLUGIN_API)
    void destroyWidgetsSoonTimerFired(Timer<Document>*);
    HashSet<RefPtr<Widget> > m_widgetsRegisteredForDestroy;
    Timer<Document> m_destroyWidgetsSoonTimer;
#endif
...
#if PLATFORM(APOLLO)
    bool m_processingDOMContentLoadedEvent;
#endif
    double m_startTime;
