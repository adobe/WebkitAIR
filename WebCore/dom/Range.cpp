...
PassRefPtr<DocumentFragment> Range::processContents(ActionType action, ExceptionCode& ec)
{
    ...
    if (m_start.container() != commonRoot && commonRoot->contains(m_start.container())) {
        ...
        for (; leftParent && leftParent != commonRoot; leftParent = leftParent->parentNode()) {
            if (action == EXTRACT_CONTENTS || action == CLONE_CONTENTS) {
                if( RefPtr<Node> leftContentsParent = leftParent->cloneNode(false) ) {
            ...
            NodeVector nodes;
            for (Node* child = m_start.container()->nextSibling(); child; child = child->nextSibling())
                nodes.append(child);
    ...
    if (m_end.container() != commonRoot && commonRoot->contains(m_end.container())) {
        ...
        for (; rightParent && rightParent != commonRoot; rightParent = rightParent->parentNode()) {
            if (action == EXTRACT_CONTENTS || action == CLONE_CONTENTS) {
                if( RefPtr<Node> rightContentsParent = rightParent->cloneNode(false) ) {
                    rightContentsParent->appendChild(rightContents, ec);
                    rightContents = rightContentsParent;
                }
            }
            NodeVector nodes;
            for (Node* child = m_start.container()->previousSibling(); child; child = child->previousSibling())
                nodes.append(child);
            for (NodeVector::const_iterator it = nodes.begin(); it != nodes.end(); it++) {
                Node* n = it->get();
                if (action == EXTRACT_CONTENTS)
    ...
    Node* n;
    if (processStart) {
        for (n = processStart.get(); n && n != processEnd; n = next) {
            next = n->nextSibling();
            if (action == EXTRACT_CONTENTS)
