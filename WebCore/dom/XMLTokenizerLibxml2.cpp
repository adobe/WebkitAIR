...
#if PLATFORM(APOLLO)
	 if (!XMLTokenizerScope::currentDocLoader->doc()->securityOrigin()->canRequestExt(url, XMLTokenizerScope::currentDocLoader->doc())) {
#else
...
#if PLATFORM(APOLLO)
            String typeAttributeValue = scriptElement->typeAttributeValue();
#endif
            if (element->dispatchBeforeLoadEvent(scriptHref) &&
#if PLATFORM(APOLLO)
                (m_pendingScript = m_doc->docLoader()->requestScript(scriptHref, scriptCharset, typeAttributeValue))) {
#else
...
// In 10.5.8 Apple changed the _xmlEntity structure and added an extra field called nbentities.
// For reference check the _xmlEntity struct in the following files linked from Apple.com. 
// In AIR we compile using an old header, meaning that the size of the statically allocated 
// memory is smaller. We need to allocate more space in order to fix this issue.
// 10.5.7: http://www.opensource.apple.com/source/libxml2/libxml2-17.7.7/libxml2/include/libxml/entities.h
// 10.5.8: http://www.opensource.apple.com/source/libxml2/libxml2-17.7.8/libxml2/include/libxml/entities.h
// 10.6: http://www.opensource.apple.com/source/libxml2/libxml2-21/libxml2/include/libxml/entities.h
struct apolloXmlEntityWrapper {
    xmlEntity entity;
    // reserve enough space for future additions
    unsigned long notUsed[4];
};
#endif
...
{
#if PLATFORM(APOLLO) && OS(DARWIN)
    static apolloXmlEntityWrapper entityWrapper;
    if (!entityWrapper.entity.type) {
        entityWrapper.entity.type = XML_ENTITY_DECL;
        entityWrapper.entity.orig = sharedXHTMLEntityResult;
        entityWrapper.entity.content = sharedXHTMLEntityResult;
        entityWrapper.entity.etype = XML_INTERNAL_PREDEFINED_ENTITY;
    }
    ASSERT(((intptr_t)&entityWrapper.entity) == ((intptr_t)&entityWrapper));
    // check that we have at least an unsigned long at the bottom of the entity
    ASSERT(sizeof(entityWrapper) >= sizeof(entityWrapper.entity) + sizeof(unsigned long));
...
#if PLATFORM(APOLLO_WINDOWS) || PLATFORM(APOLLO_UNIX)
	// Hack around libxml2's lack of encoding overide support by manually
	// resetting the encoding to UTF-16 before every chunk.  Otherwise libxml
	// will detect <?xml version="1.0" encoding="<encoding name>"?> blocks 
	// and switch encodings, causing the parse to fail.
