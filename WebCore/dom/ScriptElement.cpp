...
#if PLATFORM(APOLLO)
    ASSERT(data.element()->document());
    // It is possible to have a NULL frame for viewless documents
    // For now, we do not allow scripts to be evaluated
    KURL emptyURL;
    if (loadScriptRestricted(data.element()->document()->frame(), emptyURL))
        return;
#endif
...
void ScriptElementData::requestScript(const String& sourceUrl)
{
    ...
    m_cachedScript = document->docLoader()->requestScript(sourceUrl, scriptCharset(), m_scriptElement->typeAttributeValue());
    m_requested = true;
...
#if PLATFORM(APOLLO)
        apolloEvaluateScript(cachedScript);
#else
