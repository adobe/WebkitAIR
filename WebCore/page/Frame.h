...
namespace WebCore {
    ...
    #if (!defined(NDEBUG)) && (!PLATFORM(APOLLO))
...
#if PLATFORM(APOLLO)
public:
    WebBitmap* selectionImage(bool forceWhiteText = false) const;
    WebBitmap* snapshotDragImage(Node*, IntRect& imageRect, IntRect& elementRect) const;
private:
    WebBitmap* imageFromRect(const FloatRect&) const;
public:
#endif
