...
#endif
#if PLATFORM(APOLLO)
        // Implement to provide correct information about whether a user gesture is being processed.
        // If in doubt, return true.
        virtual bool processingUserGesture() = 0;

        // called by plugin code on platforms that support windowed plugins, when
        // a user input event is being dispatched to the plugin's window.
        virtual void startWindowedPluginUserGesture() = 0;

        // called by plugin code on platforms that support windowed plugins, when
        // the dispatch of a user input event to the plugin's window has completed.
        virtual void endWindowedPluginUserGesture() = 0;

        virtual void uncaughtJavaScriptException(const JSC::ExecState* const exec, JSC::JSValue const pExceptionValue) = 0;

        virtual WebWindow* getApolloWindow() = 0;
#endif
