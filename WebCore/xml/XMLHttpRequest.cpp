...
#if PLATFORM(APOLLO)
    , m_finishLoadingTimer(this, &XMLHttpRequest::onFinishLoadingTimer)
    , m_finishLoadingId(0)
#endif
{
    ASSERT(document());
...
#if PLATFORM(APOLLO)
    if(!scriptExecutionContext()->securityOrigin()->canRequestExt(url, document())) {
        ec = SECURITY_ERR;
        return;
    }
#endif
...
#if PLATFORM(APOLLO)
    m_sameOriginRequest = scriptExecutionContext()->securityOrigin()->canRequestExt(m_url, document());
#else
...
#if PLATFORM(APOLLO)
    //mihnea integrate -> is this still necessary?
    // Don't want to deliver async XHR before onload is delivered.
    //    Actually, this is a webkit bug, and should probably be submitted back.
    if (m_loader && document()) {
        if (document()->parsing() || document()->processingDOMContentLoadedEvent() || document()->processingLoadEvent()) {
            // Procrastinate...
            m_finishLoadingTimer.startOneShot(0.05);
            m_finishLoadingId = identifier;
            return;
        }
    }
#endif
